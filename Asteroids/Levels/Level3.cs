...
namespace Asteroids.Levels
{
    ...
    public class Level3 : GameScreen
    {
        ...
        public const int NumAsteroid1 = 4;                     //no of asteroids
        public const int NumAsteroid2 = 10;
        public const int NumAsteroid3 = 19;
        ...
        public int asteroidsonscreen = 9;

        float exTimer = 0;
        float exInterval = 1000f / 25f;
        int frameCount = 16;
        int currentFrame = 0;
        int spriteWidth = 64;
        int spriteHeight = 64;
        ...
        public override void LoadContent()
        {
            ...
            BackGround = Content.Load<Texture2D>("Images\\Asteroidbelt");
            ...
            ufoque = new float[] { 2000, 30000, 30000, 30000, 30000, 30000, 30000, 30000, 30000, 30000, 30000, 30000, 30000, 30000, 30000 };
            ...
            BallRectangle = new Rectangle(50, screenHeight-550, 150, 150);
            Hyp = new Rectangle(250, 800, 60, 65);
            ...
            asteroid1List[2] = new Asteroid(Asteroid1, AP3, 2.5f, true, 5);
            asteroid1List[3] = new Asteroid(Asteroid1, AP4, 3.005f, true, 5);

            asteroid2List[0] = new Asteroid(Asteroid2, new Vector3(2000,2000,0),0.005f,true,3);
            asteroid2List[1] = new Asteroid(Asteroid2, new Vector3(-2000,2000,0), 0.5f, true, 3);

            asteroid3List[0] = new Asteroid(Asteroid3, new Vector3(2000,1500,0), 0.5f, true, 1);
            asteroid3List[1] = new Asteroid(Asteroid3, new Vector3(-2000,-1500,0), 1.25f, true, 1);
            asteroid3List[2] = new Asteroid(Asteroid3, new Vector3(-2000, -1500, 0), 2.5f, true, 1);

            for (int i = 2; i < NumAsteroid2; i++)
                asteroid2List[i] = new Asteroid(Asteroid2, new Vector3(4000, 4000, 0), 0.005f, false, 3);

            for (int i = 3; i < NumAsteroid3; i++)
            ...
            try
            {
                if (paused == false)
                {
                    ...
                    if (asteroidsonscreen == 0 || (keyb.IsKeyDown(Keys.N)))
                    {
                        this.Content.Unload();
                        this.ExitScreen();
                        LoadingScreen.Load(ScreenManager, true, null, new WinBackgroundScreen(), new WinGameScreen());
                    }
                    #endregion
                    ...
                    if (que <= 14)
                    {
                        if (ufoque[que] == ufoTimer)
                        {
                            ...
                            Random y = new Random();
                            saucer = new UFO(ufo, new Vector3(-3500, y.Next(-2000, 2000), 0), 0.0f, 5, true);
                            que = que + 1;
                            ufoTimer = 0.0f;
                        }
                    }
