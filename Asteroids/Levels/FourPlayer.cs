...
namespace Asteroids.Levels
{
    class FourPlayer : GameScreen
    {
        ...
        Vector3 Player4Start = new Vector3(0, -2000, 0);
        #endregion
        ...
        public float spawntimer4 = 2500.0f;
            ...
            try
            {
                if (paused == false)
                {
                    ...
                    if ((p1dead == true) && (p2dead == true) && (p3dead == true))
                    {
                        this.Content.Unload();
                        this.ExitScreen();
                        LoadingScreen.Load(ScreenManager, false, null, new P4WinBGScreen(), new P4WinScreen());
                    }
                    #endregion
                    ...
                    #region Bullet 4 Timeout
                    for (int j = 0; j <= noBullets - 1; j++)
                    {
                        Bullet4List[j].AmmoModelPosition += Bullet4List[j].AmmoVelocity;
                        Bullet4List[j].timer += (float)gameTime.ElapsedGameTime.TotalMilliseconds;
                        if (Bullet4List[j].timer > interval)
                        {
                            Bullet4List[j].timer = 0.0f;
                            Bullet4List[j].AmmoFlying = false;
                        }
                    ...
                    spawntimer4 += (float)gameTime.ElapsedGameTime.Milliseconds;
        ...
        public override void Draw(GameTime gameTime)
        {
            try
            {
                if (paused == false)
                {
                    ...
                    Data2Rectangle = new Rectangle(276, 0, 168, 60);
                    Data3Rectangle = new Rectangle(564, 0, 168, 60);
                    Data4Rectangle = new Rectangle(screenWidth - 168, 0, 168, 60);
                    ...
                    if (p2dead == false)
                    {
                        ...
                        ScreenManager.SpriteBatch.DrawString(defaultFont, "Lives", new Vector2(Data2Rectangle.X + 18, 5), Color.Blue);
                        ScreenManager.SpriteBatch.DrawString(defaultFont, play2.Lives.ToString(), new Vector2(Data2Rectangle.X + 118, 5), Color.Blue);

                        ScreenManager.SpriteBatch.DrawString(defaultFont, "HyperSpace", new Vector2(Data2Rectangle.X + 18, 25), Color.Blue);
                        ScreenManager.SpriteBatch.DrawString(defaultFont, hyperSpace2.ToString(), new Vector2(Data2Rectangle.X + 138, 25), Color.Blue);
                    }
                    else
                    {
                        ScreenManager.spriteBatch.Draw(dataScreen, Data2Rectangle, Color.White);

                        ScreenManager.SpriteBatch.DrawString(defaultFont, "DEAD", new Vector2(Data2Rectangle.X + 18, 5), Color.Blue);
                    }
                    #endregion
                    ...
                    if (p3dead == false)
                    {
                        ...
                        ScreenManager.SpriteBatch.DrawString(defaultFont, "Lives", new Vector2(Data3Rectangle.X + 18, 5), Color.Yellow);
                        ScreenManager.SpriteBatch.DrawString(defaultFont, play3.Lives.ToString(), new Vector2(Data3Rectangle.X + 118, 5), Color.Yellow);

                        ScreenManager.SpriteBatch.DrawString(defaultFont, "HyperSpace", new Vector2(Data3Rectangle.X + 18, 25), Color.Yellow);
                        ScreenManager.SpriteBatch.DrawString(defaultFont, hyperSpace3.ToString(), new Vector2(Data3Rectangle.X + 138, 25), Color.Yellow);
                    }
                    else
                    {
                        ScreenManager.spriteBatch.Draw(dataScreen, Data3Rectangle, Color.White);

                        ScreenManager.SpriteBatch.DrawString(defaultFont, "DEAD", new Vector2(Data3Rectangle.X + 18, 5), Color.Yellow);
                    }
                    #endregion
                    ...
                    if (p4dead == false)
                    {
                        ScreenManager.spriteBatch.Draw(dataScreen, Data4Rectangle, Color.White);

                        ScreenManager.SpriteBatch.DrawString(defaultFont, "Lives", new Vector2(Data4Rectangle.X + 18, 5), Color.GreenYellow);
                        ScreenManager.SpriteBatch.DrawString(defaultFont, play4.Lives.ToString(), new Vector2(Data4Rectangle.X + 118, 5), Color.GreenYellow);

                        ScreenManager.SpriteBatch.DrawString(defaultFont, "HyperSpace", new Vector2(Data4Rectangle.X + 18, 25), Color.GreenYellow);
                        ScreenManager.SpriteBatch.DrawString(defaultFont, hyperSpace4.ToString(), new Vector2(Data4Rectangle.X + 138, 25), Color.GreenYellow);
                    }
                    else
                    {
                        ScreenManager.spriteBatch.Draw(dataScreen, Data4Rectangle, Color.White);

                        ScreenManager.SpriteBatch.DrawString(defaultFont, "DEAD", new Vector2(Data4Rectangle.X + 18, 5), Color.GreenYellow);
                    }
                    #endregion
                    #endregion
        ...
        public void DrawItems()
        {
            ...
            for (int j = 0; j <= noBullets - 1; j++)
            {
                if (Bullet4List[j].AmmoFlying)
                {
                    Bullet4List[j].DrawAmmo(aspectRatio, cameraPosition);
                }
            };
            #endregion
            ...
            #region Player 4
            // this if statement adds a flicker to the player when their hit
            if ((p4dead==false)&&((spawntimer4 >= spawninterval) || (spawntimer4 > 100 && spawntimer4 < 200) ||
                (spawntimer4 > 100 && spawntimer4 < 300) || (spawntimer4 > 500 && spawntimer4 < 700) ||
                (spawntimer4 > 900 && spawntimer4 < 1100) || (spawntimer4 > 1300 && spawntimer4 < 1500) ||
                (spawntimer4 > 1700 && spawntimer4 < 1900) || (spawntimer4 > 2100 && spawntimer4 < 2300)))
                play4.DrawPlayer(aspectRatio, cameraPosition);
            #endregion
        ...
        //Update data for model 4 thrust etc...
        protected void UpdateInput4()
        {
            GamePadState gpad4 = GamePad.GetState(PlayerIndex.Four);
            Vector3 velocityAdd = Vector3.Zero;

            // Find out what direction we should be thrusting, using rotation.
            velocityAdd.X = -(float)Math.Sin(play4.rotation);
            velocityAdd.Y = +(float)Math.Cos(play4.rotation);

            if (gpad4.Triggers.Left == 1.0f)
                velocityAdd *= 1.5f;
            // Finally, add this vector to our velocity.
            play4.velocity += velocityAdd;
        }
